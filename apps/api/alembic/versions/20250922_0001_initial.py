    ...
    op.execute(
        ...
        BEGIN
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'consent_type') THEN
            CREATE TYPE consent_type AS ENUM ('plaid','billing','analytics','marketing');
          END IF;
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tx_category') THEN
            CREATE TYPE tx_category AS ENUM ('housing','utilities','telco','insurance','transport','grocery','dining','entertainment','subscriptions','health','personal','fees','income','other');
          END IF;
          ...
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'suggestion_type') THEN
            CREATE TYPE suggestion_type AS ENUM ('cancel','renegotiate','switch','cap','nudge');
          END IF;
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'difficulty') THEN
            CREATE TYPE difficulty AS ENUM ('low','med','high');
          END IF;
          ...
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'export_type') THEN
            CREATE TYPE export_type AS ENUM ('pdf','email');
          END IF;
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'plan') THEN
            CREATE TYPE plan AS ENUM ('free','plus');
          END IF;
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'actor') THEN
            CREATE TYPE actor AS ENUM ('system','user','admin');
          END IF;
        END$$;
        """
    )
