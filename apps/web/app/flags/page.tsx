...
export default function FlagsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-xl font-semibold text-[var(--text)] opacity-90">Feature Flags</h1>
      ...
      <section className="rounded-lg border border-[var(--border)]/60 bg-[var(--surface)]/60 p-4">
        <div className="text-sm font-semibold text-[var(--text)] opacity-90 mb-2">Flags</div>
        <div className="divide-y divide-[var(--border)]/60">
          ...
          {Object.entries(flags).map(([k, v]) => (
            <div key={k} className="flex items-center justify-between py-2 text-sm">
              <span className="text-[var(--text)] opacity-90">{k}</span>
              <button
                onClick={() => setFlag(k, !v)}
                disabled={busy}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${v ? 'bg-brand-600' : 'bg-[var(--border)]/60'} disabled:opacity-50`}
                aria-pressed={v}
              >
                <span className={`inline-block h-5 w-5 transform rounded-full bg-[var(--text)] transition ${v ? 'translate-x-5' : 'translate-x-1'}`} />
              </button>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-lg border border-[var(--border)]/60 bg-[var(--surface)]/60 p-4 space-y-3">
        <div className="text-sm font-semibold text-[var(--text)] opacity-90">Add Flag</div>
        <div className="flex items-center gap-2">
          <input
            value={newKey}
            onChange={e => setNewKey(e.target.value)}
            placeholder="flag_key"
            className="flex-1 rounded-md bg-[var(--input)] px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-brand-600/40"
          />
